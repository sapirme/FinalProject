...
/*
    public static LinkedList<Point3D> changePoints(LinkedList<Point3D> list){
        ...
        for (Point3D p : list){
            ans.add(new Point3D(p.getX(),p.getZ(),p.getY()));
        }
        return ans;
    }
*/
    ...
    public String listToText(LinkedList<Point3D> list, double minZ, double Height) {
        ...
        d3 = new Point3D(p2.getX()- N.getX()*t,p2.getY() - N.getY()*t, p2.getZ() - N.getZ()*t);
        d4 = new Point3D(p2.getX()+ N.getX()*t,p2.getY() + N.getY()*t, p2.getZ() + N.getZ()*t);
        d5 = new Point3D(d2.getX(),d2.getY(), d2.getZ()+d);//(minZ - Height));
        d6 = new Point3D(d3.getX(),d3.getY(), d3.getZ()+d);//(minZ - Height));
        d7 = new Point3D(d4.getX(),d4.getY(), d4.getZ()+d);//(minZ - Height));
        d8 = new Point3D(d1.getX(),d1.getY(), d1.getZ()+d);//(minZ - Height));
        ans = ans + addSurface(d3, d4, d7);
        ans = ans + addSurface(d3, d6, d7);
        ans = ans + addSurface(d1, d2, d5);
        ans = ans + addSurface(d1, d8, d5);
        ...
        for(int i=0; i < list.size() - 1; i++){
            ...
            p3 = new Point3D(p1.getX(), p1.getY(), p1.getZ()+d);//(minZ - Height));
            N = getNormal(p1,p2,p3);
            t = Math.sqrt((d*d)/(N.getX()*N.getX()+N.getY()*N.getY()+N.getZ()*N.getZ()));
